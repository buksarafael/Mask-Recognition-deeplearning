...
class DetectorTrainer(pl.LightningModule):
    def __init__(self, mask_path: Path = None):
        ...
        def forward(self, x:Tensor):
            ...
            out = self.covolutional_layer2(out)
        ...
        def prepare_date(self):
            ...
            train, validate = train_test_split(mask_data_frame, test_size = 0.3, random_state = 0, stratify = mask_data_frame["mask"])
            self.train_data_frame = pd.MaskDataset(train)
            self.validate_data_frame = pd.MaskDataset(validate)
            ...
            self.crossEntropyLoss = CrossEntropyLoss(weight = Tensor.tensor(normed_weights))
